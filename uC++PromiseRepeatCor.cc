...
_CorActor Client {
	...
	#define DUP( T ) [this]( T ) { resume(); return state; }
	...
	void main() {
		for ( size_t times = 0;; times += 1 ) {
			...
			for ( size_t i = 0; i < Messages; i += 1 ) { // any promise fulfilled ?
				if ( pi[i].then( DUP( int ) ) ) results += 1;
				if ( ps[i].then( DUP( string ) ) ) results += 1;
			}
			for ( ; results < Messages * 2; results += 1 ) { // receive remaining results
				suspend();
			} // for
		  if ( times == Times - 1 ) break;
			for ( size_t i = 0; i < Messages; i += 1 ) {
				pi[i].reset();
				ps[i].reset();
			}
			results = 0;
...
// Local Variables: //
// compile-command: "u++-work -g -Wall -Wextra -O3 -nodebug -DNDEBUG -multi uC++PromiseRepeatCor.cc" //
// End: //
