				...
				if ( ExecutorMain.recs(id) >= rounds ) {
					if ( ExecutorMain.actorCnt.incrementAndGet() == ExecutorMain.Actors ) {
						if ( ExecutorMain.trials.get() > 0 ) { // ignore trial 1
							println( "Akka Executor " + f"${(System.nanoTime() - ExecutorMain.startTime) / 1_000_000_000.0}%1.2f" + "s" )
						} // if
						if ( ExecutorMain.trials.incrementAndGet() == 5 + 1 ) {
...
object ExecutorMain {
	...
	var recs = new Array[Int](Actors)
	var sends = new Array[Int](Actors)
	var groups = new Array[Int](Actors)
