...
_Actor Client {
	...
	Allocation check() {
		...
		if ( results >= 2 * Messages ) { if ( results > 2 * Messages ) osacquire( cout ) << "Fred " << results << endl; server | uActor::stopMsg; return Finished; }
	...
	Allocation receive( uActor::Message & ) {			// receive callback messages
		...
		for ( unsigned int i = 0; i < Post; i += 1 ) {	// any promise fulfilled ?
			#define COMMON posted += 1; return check();
			if ( pi[i].maybe( [this]( int ) { ints += 1; COMMON; } ) ) { numFastPath += 1; ints += 1; posted += 1; check(); }
			if ( ps[i].maybe( [this]( string ) { strs += 1; COMMON; } ) ) { numFastPath += 1; strs += 1; posted += 1; check(); }
		} // for
		return check();
	}
  public:
...
int main( int argc, char * argv[] ) {
	...
	cout << "Promise maybe server delay " << serverDelay << "  numFastPath " << numFastPath << endl;
}

// Local Variables: //
// compile-command: "u++-work -g -Wall -Wextra -O3 -nodebug -DNDEBUG -multi uC++PromiseMaybe.cc" //
// End: //
