...
class Executor( system : ActorSystem, actors : Array[ActorRef], Actors: Int, Set : Int, Rounds : Int, Processors : Int, Batch : Int, id : Int ) extends Actor {
		...
		if ( recs == rounds ) {
			if ( Executor.actorCnt.incrementAndGet() == Actors ) {
				if ( Executor.trails.get() > 0 ) { 		// ignore trial 1
					println( s"${Processors}" + " " + f"${(System.nanoTime() - Executor.startTime) / 1_000_000_000.0}%1.2f" + "s" )
				} // if
				if ( Executor.trails.incrementAndGet() == 11 + 1 ) {
                    Main.sem.release()
					system.terminate()
				} else {
...
object Executor {
	...
	def props( system : ActorSystem, actors : Array[ActorRef], Actors : Int, Set : Int, Rounds : Int, Processors : Int, Batch : Int, id : Int ) : Props
		= Props( new Executor( system, actors, Actors, Set, Rounds, Processors, Batch, id : Int ) )
}
...
object Main extends App {
	...
	for ( id <- 0 until Actors ) {						// create actors
		actors(id) = system.actorOf(
			Executor.props( system, actors, Actors, Set, Rounds, Processors, Batch, id ).withDispatcher("akka.dispatcher"), s"Executor${id}" )
	} // for
