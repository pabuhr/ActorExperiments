	...
	void main() {
		for ( unsigned int b = 0; b < (2 * Messages) / (2 * Post); b += 1 ) {
			...
			for ( unsigned int p = 0; p < Post; p += 1 ) { // any promise fulfilled ?
				...
				if ( pi[p].maybe( [this]( int ) { ints += 1; SLOW } ) ) { ints += 1; FAST }
				if ( ps[p].maybe( [this]( string ) { strs += 1; SLOW } ) ) { strs += 1; FAST }
			} // for
...
// Local Variables: //
// compile-command: "u++-work -g -Wall -Wextra -O3 -nodebug -DNDEBUG -multi uC++PromiseMaybeCor.cc" //
// End: //
